...
Third, configure credentials using either the **sidebar UI** or **Streamlit Cloud Secrets**. The updated app now has a visible **Configuration** panel in the sidebar with fields for the OpenAI API key, Apify API token, OpenAI model, Apify search actor ID, and Apify channel actor ID.

| Configuration Method | Best For | How It Works | Security Note |
|---|---|---|---|
| Sidebar UI | Fast testing after deployment | Paste keys into the sidebar fields inside the running app session. The app maps them into runtime environment variables immediately. | Keys are session-only and must be re-entered after app restart or browser/session reset. |
| Streamlit Cloud Secrets | Recommended production setup | Add keys in the app's Streamlit Cloud settings. The app reads them through `st.secrets` and maps them into environment variables at runtime.[^2] | Preferred because users do not need to paste credentials into the visible interface. |
| Local `.streamlit/secrets.toml` | Local development | Copy `.streamlit/secrets.toml.example` to `.streamlit/secrets.toml` and add real keys locally. | The real `secrets.toml` file is ignored by Git and should never be committed. |

Use the Streamlit Cloud Secrets format below when you want persistent server-side configuration.
...
|---|---|---|
| Generate keyword suggestions | `OPENAI_API_KEY` from sidebar UI or Streamlit Secrets | Reviewed keyword JSON |
| Scrape YouTube via Apify | `APIFY_TOKEN` from sidebar UI or Streamlit Secrets | Raw Apify JSON and deduplicated channel CSV |
| Score imported channels with AI | `OPENAI_API_KEY` from sidebar UI or Streamlit Secrets | Ranked CSV |
| Score imported channels without AI | None | Heuristic ranked CSV |
| Enrich imported channels via Apify | `APIFY_TOKEN` from sidebar UI or Streamlit Secrets | Enriched and ranked CSV |
...
If you do not see AI or Apify functions working after deployment, open the app sidebar and check **Credential status**. You can either paste credentials directly into the sidebar for the current session or configure them permanently in Streamlit Cloud Secrets.
