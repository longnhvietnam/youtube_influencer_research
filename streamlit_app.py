...
def clear_ui_entered_api_keys() -> None:
    """Clear API keys entered through the sidebar and fall back to configured secrets."""
    os.environ.pop("OPENAI_API_KEY", None)
    os.environ.pop("APIFY_TOKEN", None)
    st.session_state["ui_openai_api_key"] = ""
    st.session_state["ui_apify_token"] = ""
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.caption(
            "Configure API keys here for the current session, or set them permanently in Streamlit Community Cloud Secrets."
        )

        with st.expander("API keys and actor settings", expanded=True):
            st.markdown(
                "Values entered here are used only by this running app session. For production deployments, "
                "use Streamlit Cloud Secrets so users do not need to paste keys into the interface."
            )
            openai_api_key = st.text_input(
                ...
                help="Required for AI keyword generation and AI scoring.",
            )
            apify_token = st.text_input(
                ...
                type="password",
                key="ui_apify_token",
                help="Required for YouTube scraping and optional channel enrichment.",
            )
            openai_model = st.text_input(
                "OpenAI model",
                value=os.getenv("OPENAI_MODEL", core.DEFAULT_OPENAI_MODEL),
                key="ui_openai_model",
            )
            apify_search_actor_id = st.text_input(
                "Apify search actor ID",
                value=os.getenv("APIFY_SEARCH_ACTOR_ID", core.DEFAULT_SEARCH_ACTOR_ID),
                key="ui_apify_search_actor_id",
            )
            apify_channel_actor_id = st.text_input(
                "Apify channel actor ID",
                value=os.getenv("APIFY_CHANNEL_ACTOR_ID", core.DEFAULT_CHANNEL_ACTOR_ID),
                key="ui_apify_channel_actor_id",
            )

            if openai_api_key.strip():
                os.environ["OPENAI_API_KEY"] = openai_api_key.strip()
            if apify_token.strip():
                os.environ["APIFY_TOKEN"] = apify_token.strip()
            if openai_model.strip():
                os.environ["OPENAI_MODEL"] = openai_model.strip()
            if apify_search_actor_id.strip():
                os.environ["APIFY_SEARCH_ACTOR_ID"] = apify_search_actor_id.strip()
            if apify_channel_actor_id.strip():
                os.environ["APIFY_CHANNEL_ACTOR_ID"] = apify_channel_actor_id.strip()

            st.button("Clear UI-entered API keys", on_click=clear_ui_entered_api_keys)

        st.divider()
        st.markdown("**Credential status**")
        status_df = pd.DataFrame(
            [
                {"Setting": "OPENAI_API_KEY", "Available": secret_available("OPENAI_API_KEY")},
                {"Setting": "APIFY_TOKEN", "Available": secret_available("APIFY_TOKEN")},
                {"Setting": "OPENAI_MODEL", "Available": secret_available("OPENAI_MODEL")},
                {"Setting": "APIFY_SEARCH_ACTOR_ID", "Available": secret_available("APIFY_SEARCH_ACTOR_ID")},
                {"Setting": "APIFY_CHANNEL_ACTOR_ID", "Available": secret_available("APIFY_CHANNEL_ACTOR_ID")},
            ]
        )
        st.dataframe(status_df, use_container_width=True, hide_index=True)
        st.divider()
        st.markdown(
            "**Tip.** You can score imported channels without an OpenAI key by disabling AI scoring. "
            "Apify scraping requires an Apify API token."
        )
        st.divider()
...
def keyword_builder_tab() -> None:
    ...
    if st.button("Generate keyword suggestions", type="primary"):
        if not brief.strip():
            ...
            st.error("OPENAI_API_KEY is not configured. Add it in the sidebar API key field or in Streamlit Cloud Secrets to use AI keyword generation.")
        else:
...
def scrape_tab() -> None:
    ...
    if st.button("Run Apify scrape", type="primary"):
        ...
        if not keywords:
            ...
            st.error("APIFY_TOKEN is not configured. Add it in the sidebar API token field or in Streamlit Cloud Secrets before scraping.")
        else:
...
def import_and_score_tab() -> None:
    ...
    if st.button("Score and rank channels", type="primary"):
        if not channels:
            ...
            st.error("OPENAI_API_KEY is not configured. Disable AI scoring, add it in the sidebar API key field, or set it in Streamlit Cloud Secrets.")
        elif enrich and not secret_available("APIFY_TOKEN"):
            st.error("APIFY_TOKEN is required for enrichment. Disable enrichment, add it in the sidebar API token field, or set it in Streamlit Cloud Secrets.")
        else:
...
def full_workflow_tab() -> None:
    ...
    if st.button("Run full workflow", type="primary"):
        ...
        if not brief.strip():
            ...
            st.error("APIFY_TOKEN is required for scraping. Add it in the sidebar API token field or set it in Streamlit Cloud Secrets.")
        elif use_ai and not secret_available("OPENAI_API_KEY"):
            st.error("OPENAI_API_KEY is required for AI scoring. Disable AI scoring, add it in the sidebar API key field, or set it in Streamlit Cloud Secrets.")
        else:
            try:
